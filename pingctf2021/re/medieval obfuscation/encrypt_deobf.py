...
with open("flag.txt", "rb") as flag_txt:
	encoded_flag = b"".join(flag_txt.readlines())[::-int(str(obg(DWdFaFXGwROWZKun,iJCxBYGeIazlujbH,VXbYeaXGWDsvCNCz,OUOjdPHXwXtOYfKF,AwzkEKbzAMUeLmYr,HzyGlHuvFlQdOhZd))[:int(obg(DWdFaFXGwROWZKun,iJCxBYGeIazlujbH,VXbYeaXGWDsvCNCz,OUOjdPHXwXtOYfKF,AwzkEKbzAMUeLmYr,HzyGlHuvFlQdOhZd))])]
	with open("encoded.txt", "wb") as encoded_txt_file:
		encrypted_flag = b""
		for i in range(len(encoded_flag)):
			encrypted_flag += bytes([int(encoded_flag[i])^RiWdagGgRfVvwHIJ(i)])
		encoded_txt_file.write(encrypted_flag)